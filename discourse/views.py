...
def vote(request):
    if request.method == 'POST':
        ...
        try:
            ...
            vote = CommentVote(user=user, comment=comment)
        if direction == '-1':
            vote.value = -1
        else:
            vote.value = 1
        for reciever, response in attachment_view.send(sender=attachment, request=request, vote=vote):
            if isinstance(response, HttpResponse):
                return response
        vote.save()
        comment.save()
        return JsonResponse(vote.value)
